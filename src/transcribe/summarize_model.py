...
Use Markdown format for the output, please add colors and other text formats for better representation

For example, if a speaker claims that "a dosage of X increases Y", do not
just write "the speaker discusses the effects of X", instead write "a dosage 
of X increases Y".
...
def count_tokens(text):
    ...
    tokens = st.session_state.tokenizer.encode(text)
    return len(tokens)


# Load MLX model and tokenizer
# model, tokenizer = load(f"/Users/gmcjy/Documents/sourcecode/github/mlx-community/Mistral-7B-Instruct-v0.2-4bit-mlx")
# model, tokenizer = load(f"mlx-community/Mistral-7B-Instruct-v0.2-4bit-mlx")
...
def summarize_with_mlx(chunk=None, title=None):
    ...
    return generate(st.session_state.model, st.session_state.tokenizer,
                    prompt=prompt,
                    max_tokens=MODEL_MAX_TOKENS - count_tokens(prompt))
